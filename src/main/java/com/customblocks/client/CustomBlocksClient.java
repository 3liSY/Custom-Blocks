...
public class CustomBlocksClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        // Load blocks from client config on startup
        CustomBlocksMod.loadBlocksFromConfig();

        ClientLifecycleEvents.CLIENT_STARTED.register(client -> {
            if (!CustomBlocksMod.CUSTOM_BLOCKS.isEmpty() || CustomBlocksMod.TAB_ICON_BLOCK != null) {
                ResourcePackGenerator.generate();
                injectPackIfNeeded(client);
            }
        });

        // Receive full block list from server — delete any stale local blocks not on server
        ClientPlayNetworking.registerGlobalReceiver(CustomBlockListPayload.ID,
            (payload, context) -> {
                List<String> serverIds = payload.blockIds();
                context.client().execute(() -> cleanStaleBlocks(context.client(), serverIds));
            }
        );

        // Receive individual block sync (new block or on-join resync)
        ClientPlayNetworking.registerGlobalReceiver(CustomBlockSyncPayload.ID,
            (payload, context) -> {
                ...
                context.client().execute(() -> {
                    try {
                        handleBlock(context.client(), blockId, displayName, texture);
                    } catch (Exception e) {
     ...
     * so they don't show as ghost items in the creative tab.
     */
    private static void cleanStaleBlocks(MinecraftClient client, List<String> serverIds) {
        ...
        for (File folder : folders) {
            ...
            if (!serverIds.contains(blockId)) {
                deleteFolder(folder);
                CustomBlocksMod.LOGGER.info("[CustomBlocks] Removed stale block: {}", blockId);
                changed = true;
            }
        }

        if (changed) {
            // Regenerate resource pack without the deleted blocks
            ResourcePackGenerator.generate();
        }
    }
    ...
    /**
     * Register a block on the client (if not already registered) and save its texture.
     */
    private static void handleBlock(MinecraftClient client, String blockId,
